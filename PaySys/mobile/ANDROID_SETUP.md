Android setup and build instructions for PaySys mobile
...
1. From `PaySys/mobile` ensure `node_modules` are installed: `yarn` or `npm install`.
2. Generate Gradle wrapper (if you don't have `gradle`): from `PaySys/mobile/android` run `gradle wrapper` or if you have `gradlew` already use it.
   - On Windows: run `gradlew.bat assembleDebug` from `PaySys/mobile/android` (or `gradlew.bat -v` to verify).
3. Build debug APK: from `PaySys/mobile/android` run `./gradlew assembleDebug` (or `gradlew.bat assembleDebug` on Windows).
