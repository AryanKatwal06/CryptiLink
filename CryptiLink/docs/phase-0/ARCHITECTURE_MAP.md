...
The repository is a nested monorepo-style workspace. The top-level repository is a wrapper around the actual CryptiLink workspace in `CryptiLink/`.

- Top-level package manager: npm
- Lockfile format: package-lock.json lockfileVersion 3
- Workspace manager in the CryptiLink workspace: Turborepo (`turbo.json`)
- Declared workspace layout in `CryptiLink/package.json`: `apps/*`, `packages/*`
...
- `CryptiLink/package.json` and the repository README describe `apps/` and `packages/`, but the actual source folders are `mobile/`, `backend/`, and `shared/`.
...
No runtime mobile dependency manifest exists under `CryptiLink/mobile/`, so React Native / navigation / state / networking package versions are not directly confirmable from a package file.
...
- `docker-compose.yml` - top-level compose wrapper for CryptiLink
...
- `CryptiLink/` - actual product workspace

### CryptiLink workspace root
...
- `docker-compose.yml` - CryptiLink runtime compose file
...
| ------------------------------ | ----------- | ---------------------------------------------- | -------------------------------------------------------------------------------------------------------- | -------------------------------------- |
| `app/index.tsx`                | small       | Native entrypoint; registers CryptiLink app        | react-native, `./StartupCoordinator`                                                                     | default export `App`                   |
...
CryptiLink routes that do not exist yet and will need to be added:
...
CryptiLink-required stores that are missing:
...
- `CryptiLink/.env.example` defines `API_URL`, `DATABASE_URL`, `REDIS_URL`, `JWT_SECRET`, and `ENVIRONMENT`.
