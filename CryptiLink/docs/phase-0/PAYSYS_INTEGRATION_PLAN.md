# CryptiLink - Integration Plan

# How new CryptiLink features integrate with existing codebase

## 1. What We Keep (Unchanged)

- `CryptiLink/shared/contracts/*` as the shared DTO and enum source of truth.
- `CryptiLink/shared/types/index.ts` as the common result/error envelope layer.
- `CryptiLink/shared/constants/index.ts` as the initial route/storage/network constant namespace.
- `CryptiLink/mobile/theme/tokens.ts` as the dark-first token foundation.
- `CryptiLink/mobile/theme/registry.ts` as the token access wrapper, once it is expanded rather than replaced.
- `CryptiLink/mobile/tests/run-tests.js` as a lightweight smoke gate until proper test infrastructure is introduced.
- `CryptiLink/backend/src/main/java/com/payments/backend/health/HealthController.java` as the basic operational health surface.
- `CryptiLink/backend/src/main/java/com/payments/backend/config/ConfigLoader.java` as the environment loading entrypoint, once key names are standardized.
- `CryptiLink/backend/src/main/java/com/payments/backend/errors/GlobalExceptionHandler.java` as the global error envelope pattern.
